...
public class Scheduler {
    ...
    public void handleJobSubmission(TSchedulingRequest request) throws TException {
        ...
        //The beginning time-stamp for handling a submitted job!!
        long start = System.currentTimeMillis();

        String requestId = getRequestId();

        //Init records for the request
        LOG.debug("Sparrow start handling request: " + requestId + " at time stamp: " + start);
        ...
        for (Entry<InetSocketAddress, TEnqueueTaskReservationsRequest> entry :
            ...
            try {
                ...
                LOG.debug("Launching enqueueTask for request " + requestId + "on node: " + entry.getKey() + "; Tasks reservation enqueued: " + entry.getValue().numTasks);
                AUDIT_LOG.debug(Logging.auditEventString(
